...
getMetMatchResults = function(metFiles,cutoff=0){
   ...
   ##################################################################
   df = data.frame(ID ,mz,rt,int,formula,score)
...
#' @keywords internal
#'
groupIonsByRT = function(results,maxRtDiff = 20){
...
mergeData = function(g.ions,which = "intensity"){
   ...
   out = data.frame()
   form = featID = mn.rt=mn.mz= mn.score = cv.score= mn.int = cv.int = n.samples = array()
   ...
   colnames(out) =tm$ID
   meta = data.frame(ID = featID,M = as.numeric(mn.mz),rt = as.numeric(mn.rt),int = as.numeric(mn.int),
...
#' @export
#'
align.MetMatchResults = function(wd,cutoff=0.1,N=1,maxRtDiff=20){
   startwd = getwd()
   setwd(wd)
